  ...
  <div class="page-container">
    <div class="page-toolbar">
      <el-button type="primary" @click="showDialog(null)"
        ><el-icon><Plus /></el-icon> 新建收款记录</el-button
      >
      <el-select
        ...
        @change="fetchData"
      >
        <el-option label="未到账" value="未到账" /><el-option
          label="已到账"
          value="已到账"
    ...
    <el-row :gutter="16" class="mb-4">
      <el-col :span="6"
        ><div class="stat-card">
          ...
          <div class="stat-val">¥{{ formatMoney(summary.total_amount) }}</div>
        </div></el-col
      >
      <el-col :span="6"
        ><div class="stat-card" style="border-left:3px solid var(--color-success)">
          <div class="stat-label">已到账</div>
          <div class="stat-val" style="color:var(--color-success)">
      ...
      <el-col :span="6"
        ><div class="stat-card" style="border-left:3px solid var(--color-danger)">
          <div class="stat-label">未到账</div>
          <div class="stat-val" style="color:var(--color-danger)">
    ...
    <el-table v-loading="loading" :data="tableData" stripe border>
      ...
      <el-table-column label="订单 出货/总量" width="100" align="center">
        <template #default="{row}"
          ><span style="color:var(--color-success)">{{ row.shipped_qty }}</span
          ><span style="color:var(--color-text-tertiary)">/</span>{{ row.order_qty }}</template
        >
      </el-table-column>
      ...
      <el-table-column label="状态" width="100">
        <template #default="{row}">
          <el-tag
            :type="row.status==='已到账'?'success':row.status==='部分到账'?'warning':'danger'"
            size="small"
            >{{ row.status }}</el-tag
          >
        </template>
        ...
        <el-form-item label="销售订单" required>
          ...
          >
            <el-option
              ...
              :key="o.id"
              :label="`${o.order_number} - ${o.customer_name} ${o.product_name||''}`"
              :value="o.id"
            />
          </el-select>
        </el-form-item>
        ...
        <el-row :gutter="16">
          <el-col :span="12"
            ><el-form-item label="金额" required
              ><el-input-number v-model="form.amount" :min="0" class="w-full" /></el-form-item
          ></el-col>
          <el-col :span="12"
            ><el-form-item label="状态"
              ><el-select v-model="form.status"
                ><el-option label="未到账" value="未到账" /><el-option
                  ...
                  value="已到账" /><el-option
                  label="部分到账"
                  value="部分到账" /></el-select></el-form-item
          ></el-col>
        </el-row>
        <el-row :gutter="16">
          <el-col :span="12"
            ...
            ><el-form-item label="付款方式"
              ><el-select v-model="form.payment_method"
                ><el-option label="银行转账" value="银行转账" /><el-option
                  label="现金"
                  value="现金" /><el-option label="微信" value="微信" /><el-option
                  label="支付宝"
                  value="支付宝" /></el-select></el-form-item
          ></el-col>
        </el-row>
