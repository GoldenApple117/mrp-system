<template>
  <div class="page-container">
    <el-card style="margin-bottom:16px">
      <template #header><span style="font-weight:500">CRP 产能需求计划</span></template>
      <el-row :gutter="16" align="middle">
        <el-col :span="6">
          <span style="color:#606266">展望期(周)：</span>
          <el-input-number v-model="horizonWeeks" :min="2" :max="52" />
        </el-col>
        <el-col :span="8">
          <el-button type="primary" size="large" @click="calculateCrp" :loading="loading">
            计算产能负荷
          </el-button>
          <el-text type="info" style="margin-left:12px">基于工单+工艺路线→工作中心负荷</el-text>
        </el-col>
      </el-row>
    </el-card>
    ...
    <!-- 瓶颈工作中心 -->
    <el-card v-if="bottlenecks.length" style="margin-bottom:16px">
      <template #header><span style="color:#f56c6c;font-weight:500">瓶颈工作中心 ({{ bottlenecks.length }})</span></template>
      <el-table :data="bottlenecks" stripe border size="small">
        ...
        <el-table-column label="利用率" width="120">
          <template #default="{row}">
            <el-progress :percentage="Math.min(row.utilization_pct,200)" :color="row.utilization_pct>130?'#f56c6c':'#e6a23c'" />
          </template>
        </el-table-column>
    ...
    <el-card v-if="loadAnalysis.length">
      <template #header>
        <span style="font-weight:500">产能负荷明细</span>
        <el-tag style="margin-left:8px" size="small">利用率 > 100% = 红色</el-tag>
      </template>
      <el-table :data="loadAnalysis" stripe border size="small" max-height="500">
        ...
        <el-table-column label="利用率" width="180">
          <template #default="{row}">
            <el-progress :percentage="Math.min(row.utilization_pct,200)" :color="utilColor(row.utilization_pct)" :text-inside="true" :stroke-width="18" />
          </template>
        </el-table-column>
        <el-table-column label="状态" width="100">
          <template #default="{row}">
            <el-tag :type="row.is_overloaded?'danger':'success'" size="small">{{ row.is_overloaded?'超载':'正常' }}</el-tag>
          </template>
        </el-table-column>
      </el-table>
    </el-card>
