  ...
  <div class="page-container space-y-4">
    ...
    <div class="page-toolbar">
      ...
      <el-tag
        ...
        closable
        class="ml-1"
        @close="keyword='';fetchData()"
      >
        "{{ keyword }}"
      </el-tag>
      <span class="flex-1"></span>
      <el-button type="primary" @click="showDialog()"
        ><el-icon><Plus /></el-icon> 新增物料</el-button
      >
    </div>
    ...
    <div v-if="selIds.length" class="batch-bar">
      ...
      <el-button
        size="small"
        type="primary"
        :disabled="!batchPrice && batchPrice !== 0"
        @click="batchUpdate('reference_unit_price',batchPrice,'单价')"
        >更新单价</el-button
      >
      <div class="w-px h-4 bg-[var(--color-border-light)]"></div>
      <div class="flex items-center gap-1.5 text-xs text-[var(--color-text-tertiary)]">
        <span>提交人</span>
        <el-input v-model="batchSubmitter" size="small" class="w-[80px]" placeholder="提交人" />
      </div>
      <el-button
        size="small"
        type="primary"
        :disabled="!batchSubmitter"
        @click="batchUpdate('reference_submitter',batchSubmitter,'提交人')"
        >更新提交人</el-button
      >
      <span class="flex-1"></span>
      <el-button size="small" text @click="selIds=[];batchPrice=null;batchSubmitter=''"
        >取消选择</el-button
      >
    </div>
              ...
              <div class="mod-header" @click="m._open = !m._open">
                ...
                <div class="flex items-center gap-3">
                  <span class="text-xs text-[var(--color-text-tertiary)]"
                    >{{ filterParts(m.parts).length }} 项零件</span
                  >
                  <el-icon
                    :size="11"
                    color="var(--color-text-tertiary)"
                ...
                >
                  ...
                  <el-table-column label="参考单价" width="90" align="right">
                    <template #default="{ row }">
                      <span v-if="row.reference_unit_price > 0" class="tabular-nums"
                        >¥{{ row.reference_unit_price.toFixed(2) }}</span
                      >
                  ...
                  <el-table-column label="采购件" width="65" align="center">
                    <template #default="{ row }">
                      <el-tag
                        :type="row.is_purchased ? 'success' : 'info'"
                        size="small"
                        effect="dark"
                      >
        ...
        <el-row :gutter="14">
          <el-col :span="8">
            <el-form-item label="提前期"
              ><el-input-number v-model="form.lead_time" :min="0" class="w-full"
            /></el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="安全库存"
              ><el-input-number v-model="form.safety_stock" :min="0" class="w-full"
            /></el-form-item>
          </el-col>
          <el-col :span="8">
